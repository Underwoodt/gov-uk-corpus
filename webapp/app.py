...
_PROMPT_REVIEW_SYSTEM = (
    ...
    "## Paste-ready edits\n"
    "For EACH field that changes, output a `### <field>` heading — one of `### Exclude criteria`, "
    "`### Keep examples`, `### Drop examples`, `### Include criteria` — then the full revised text for "
    "THAT FIELD ONLY in a fenced ```code block```, followed by one short bullet of rationale. Omit any "
    "field that needs no change. Each block is pasted verbatim into ONE input field, so it must contain "
    "ONLY that field's content — no internal labels, and no contrasting sections. The base template "
    "already labels the fields, so: the Keep examples block lists ONLY pages to KEEP (short bullets, no "
    "'do not keep' lines); the Drop examples block lists ONLY pages to DROP; the Exclude criteria block "
    "is ONLY the exclusion rule text. Never put a KEEP / DO NOT KEEP / DROP sub-heading inside a block.\n"
